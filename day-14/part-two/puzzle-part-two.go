...
**/
// Look for the smallest safety factor
	...
	total := simulateAndCalculateSafetyFactor(initialSpace, &robots)
...
func simulateAndCalculateSafetyFactor(space [][]int, robots *[]Robot) int {

	seconds := 0
	// Place robots on the space
	...
	safetySecond := 0
	lowestSafety := 1000000000000000
	// Simulate robot movement 
	for range 50000{
		seconds++
		// fmt.Printf("\n\n********** ROBOT %s simulation********\n", robot.name)
		for j := range *robots {
			simulateRobotMovement(&space, &(*robots)[j])
		}
		safety := calculateSafetyFactorWithoutMiddle(space)

		if safety < lowestSafety {
			safetySecond = seconds
			lowestSafety = safety
		}
		...
		// fmt.Printf("\nSAFETY : %d, SECONDS : %d", safety, seconds)
	}
	return safetySecond
}
...
func dealWithPositionOverlapping(space [][]int, pos Position) Position {
	...
	// upper border
	if pos.X < 0 {
		fixedPosition.X = len(space) + fixedPosition.X
	}

	// lower border
	if pos.X >= len(space) {
		fixedPosition.X = fixedPosition.X - len(space)
	}

	// right border
	if pos.Y >= len(space[0]) {
		fixedPosition.Y = fixedPosition.Y - len(space[0])
	}

	// left border
	if pos.Y < 0 {
		fixedPosition.Y = len(space[0]) + fixedPosition.Y
	}
